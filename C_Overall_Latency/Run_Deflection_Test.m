...
scenarios(end + 1) = scenarioRow( ...
    "fast_coarse", 1.25, [0, 0.25, 0.50, 0.75, 1.00], 0.015, ...
    "Short holds with coarse steps to expose settling limits.");
scenarios(end + 1) = scenarioRow( ...
    "medium_nominal", 1.50, [0, 0.20, 0.40, 0.60, 0.80, 1.00], 0.02, ...
    "Moderate holds with nominal levels.");
scenarios(end + 1) = scenarioRow( ...
    "baseline_nominal", 2.00, [0, 0.20, 0.40, 0.60, 0.80, 1.00], 0.025, ...
    "Current baseline calibration.");
scenarios(end + 1) = scenarioRow( ...
    "slow_nominal", 2.50, [0, 0.20, 0.40, 0.60, 0.80, 1.00], 0.030, ...
    "Longer holds to reduce settling sensitivity.");
scenarios(end + 1) = scenarioRow( ...
    "fine_deadband", 2.00, [0, 0.05, 0.10, 0.15, 0.20, 0.30, 0.45, 0.60, 0.80, 1.00], 0.020, ...
    "Finer low-end levels to estimate discrete/deadband behavior.");
end
