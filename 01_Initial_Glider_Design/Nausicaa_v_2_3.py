...
htail_span       = 0.5 * AR_ht * htail_root_chord * (1 + taper_ht)
...
vtail_root_chord = opti.variable(init_guess = 0.07, lower_bound = 1e-3,)
vtail_span       = 0.5 * AR_vt * vtail_root_chord * (1 + taper_vt)
...
obj_span = 0.2 * (wing_span + htail_span + vtail_span)

### Objective
objective = obj_sink + obj_mass + obj_span
...
if __name__ == '__main__': # only run this block when the file is executed directly
    ...
    check_var("htail_root_chord",  htail_root_chord,           lb = 1e-3,   ub = wing_root_chord)

    check_var("l_vt",              l_vt,                       lb = 0.2,    ub = 1.5)
    check_var("vtail_root_chord",  vtail_root_chord,           lb = 1e-3,   ub = wing_root_chord)
    ...
    design_results = {
        ...
        "V_ht"               : to_scalar(V_ht),
        "V_vt"               : to_scalar(V_vt),
        "Re_w"               : to_scalar(eng_string(op_point.reynolds(sol(wing.mean_aerodynamic_chord())))),
