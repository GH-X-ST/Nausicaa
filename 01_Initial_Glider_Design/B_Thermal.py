...
if __name__ == "__main__" and make_plots:
    ...
    cbar_2d = fig_2d.colorbar(contour, ax=ax2d)
    cbar_2d.set_label(
        f"vertical velocity, w (m/s), at z = {z_th_slice:.2f} m"
    )
    ...
    fig_2d.tight_layout()
    fig_2d.savefig(
        os.path.join(output_dir, "thermal_slice.png"),
        dpi=300,
        bbox_inches="tight",
    )
    ...
    # 3D circle showing initial thermal core radius R_th0 at z0
    ...
    # light grid styling (uses private API, but ok for plots)
    ...
    fig_3d.tight_layout()
    fig_3d.savefig(
        os.path.join(output_dir, "thermal_plume.png"),
        dpi=300,
        bbox_inches="tight",
    )
