...
def extract_param_arrays(
    ...
    if missing:
        raise ValueError(f"Missing required per-fan columns: {missing}")

    local = df[required_cols].copy()
    for col in required_cols:
        local[col] = pd.to_numeric(local[col], errors="coerce")

    local = local.dropna(subset=required_cols)
    if local.empty:
        raise ValueError("No valid per-fan parameter rows after cleaning.")
...
def main() -> None:
    ...
    z_vals, params_stack = extract_param_arrays(params_df, fan_ids=fan_ids)
