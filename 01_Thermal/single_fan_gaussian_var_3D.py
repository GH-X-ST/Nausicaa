...
The model is
    w_model(r, z) = A(z) * exp(-(r / delta(z))**2) [+ w0(z), optional]
where [A, delta, w0] are read directly from the precomputed PCHIP table
written by single_fan_gaussian_var_fit.py.
"""
...
PARAMS_XLSX = Path("B_results/single_var_params_pchip.xlsx")
PARAMS_SHEET = "single_var_pchip"

OUT_DIR = Path("A_figures/Single_Fan_Gaussian_Var")
OUT_3D_NAME = "single_gaussian_var_3d.png"
...
# Typography (match single_fan_gaussian_var_heat_map_main.py)
...
# Keep False by default to match previous visual convention.
INCLUDE_W0 = True
...
def extract_param_arrays(
    ...
    a_vals = local["A"].to_numpy(dtype=float)
    delta_vals = local["delta"].to_numpy(dtype=float)
    ...
    return z_vals, a_vals, delta_vals, w0_vals
...
def evaluate_field(
    ...
    if np.any(mask):
        ...
        a_eval = np.interp(z_eval, z_vals, a_vals)
        delta_eval = np.interp(z_eval, z_vals, delta_vals)
...
def main() -> None:
    ...
    z_vals, a_vals, delta_vals, w0_vals = extract_param_arrays(params_df)
