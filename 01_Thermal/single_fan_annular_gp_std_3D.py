from __future__ import annotations

from pathlib import Path
from typing import Sequence, Tuple

import cmocean
import matplotlib.colors as mcolors
import numpy as np

import single_fan_gp_3D as base_3d


SHEETS = ["z020", "z035", "z050", "z075", "z110", "z160", "z220"]
GRID_PRED_XLSX = Path(
    "B_results/Single_Fan_Annular_GP/single_annular_gp_grid_predictions.xlsx"
)
OUT_DIR = Path("A_figures/Single_Fan_Annular_GP")
OUT_3D_NAME = "single_annular_gp_std_3d.png"

CBAR_VMIN = 0.0
CBAR_VMAX = 0.4
CBAR_TICK_STEP = 0.1
CBAR_LABEL = r"$\sigma_{\mathrm{res}}$ (m $\!$s$^{-1}$)"


def build_alpha_cmap() -> mcolors.ListedColormap:
    """Build a deep colormap with exponential alpha scaling."""
    base_cmap = cmocean.cm.deep
    colors = base_cmap(np.linspace(0.0, 1.0, 256))

    t_norm = np.linspace(0.0, 1.0, colors.shape[0])
    exp_scale = np.exp(base_3d.ALPHA_EXP_RATE * t_norm)
    exp_full = np.exp(base_3d.ALPHA_EXP_RATE)
    alpha = (exp_scale - 1.0) / (exp_full - 1.0)
    alpha[0] = 0.0
    alpha[-1] = 1.0
    colors[:, 3] = alpha

    return mcolors.ListedColormap(colors)


def load_annular_gp_std_stack(
    xlsx_path: Path,
    sheet_names: Sequence[str],
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    x_ref = None
    y_ref = None
    z_list = []
    w_layers = []

    for sheet_name in sheet_names:
        x, y, w = base_3d.load_gp_mean_sheet(
            xlsx_path,
            f"{sheet_name}_annular_gp_std",
        )
        if x_ref is None:
            x_ref = x
            y_ref = y
        else:
            if not np.allclose(x, x_ref, atol=1e-9, rtol=0.0):
                raise ValueError(
                    (
                        "Inconsistent x-grid found in sheet "
                        f"'{sheet_name}_annular_gp_std'."
                    )
                )
            if not np.allclose(y, y_ref, atol=1e-9, rtol=0.0):
                raise ValueError(
                    (
                        "Inconsistent y-grid found in sheet "
                        f"'{sheet_name}_annular_gp_std'."
                    )
                )

        z_list.append(base_3d.parse_sheet_height_m(sheet_name))
        w_layers.append(w)

    z_axis = np.asarray(z_list, dtype=float)
    order = np.argsort(z_axis)
    z_axis = z_axis[order]
    w_stack = np.stack([w_layers[idx] for idx in order], axis=0)
    return x_ref, y_ref, z_axis, w_stack


def main() -> None:
    if not base_3d.MAKE_PLOTS:
        print("MAKE_PLOTS is False; nothing to do.")
        return
    if not GRID_PRED_XLSX.exists():
        raise FileNotFoundError(
            f"Missing annular-GP grid workbook: {GRID_PRED_XLSX}"
        )

    x_axis, y_axis, z_axis, w_stack = load_annular_gp_std_stack(
        xlsx_path=GRID_PRED_XLSX,
        sheet_names=SHEETS,
    )
    x_grid, y_grid, z_grid, dx, dy, dz = base_3d.make_3d_grid()
    z_query = np.linspace(
        base_3d.REL_Z_EXTRAP_MIN,
        base_3d.REL_Z_EXTRAP_MAX,
        z_grid.shape[2],
        dtype=float,
    )
    w_stack_extrap = base_3d.extrapolate_stack_in_z_with_gp(
        z_axis=z_axis,
        w_stack=w_stack,
        z_query=z_query,
    )
    z_grid_rel = z_grid - base_3d.FAN_VERTICAL_OFFSET_M
    w_grid = base_3d.evaluate_field(
        x_grid=x_grid,
        y_grid=y_grid,
        z_grid=z_grid_rel,
        x_axis=x_axis,
        y_axis=y_axis,
        z_axis=z_query,
        w_stack=w_stack_extrap,
    )

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    out_3d = OUT_DIR / OUT_3D_NAME
    base_3d.CBAR_VMIN = float(CBAR_VMIN)
    base_3d.CBAR_VMAX = float(CBAR_VMAX)
    base_3d.CBAR_TICK_STEP = float(CBAR_TICK_STEP)
    base_3d.CBAR_LABEL = CBAR_LABEL
    base_3d.plot_isosurfaces(
        w_grid=w_grid,
        dx=dx,
        dy=dy,
        dz=dz,
        cmap_alpha=build_alpha_cmap(),
        output_path=out_3d,
    )
    print(f"Saved 3D annular-GP std plot to: {out_3d.resolve()}")


if __name__ == "__main__":
    main()
