...
def nearest_fan_radius_map(
    x_grid: np.ndarray,
    y_grid: np.ndarray,
    fan_outlet_points: Tuple[Tuple[float, float], ...],
) -> np.ndarray:
    """
    Compute nearest-fan radius for each 3D grid sample.
    """
    fan_xy = np.asarray(fan_outlet_points, dtype=float)
...
def evaluate_field(
    ...
    """
    r_grid = nearest_fan_radius_map(
        x_grid=x_grid,
        y_grid=y_grid,
        fan_outlet_points=tuple(FAN_OUTLET_POINTS),
    )
