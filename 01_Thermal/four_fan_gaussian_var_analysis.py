...
def params_by_fan_at_z(
    ...
    """
    Interpolate [A, delta, w0] at one target height for each fan.
    """
    ...
    if len(fan_ids) == 0:
        ...
        a = pd.to_numeric(fit_df["A"], errors="coerce").to_numpy(dtype=float)[order]
        d = pd.to_numeric(fit_df["delta"], errors="coerce").to_numpy(dtype=float)[order]
    ...
    for fan_idx, fan_id in enumerate(fan_ids):
        a = pd.to_numeric(fit_df[f"A_{fan_id}"], errors="coerce").to_numpy(dtype=float)[order]
        d = pd.to_numeric(fit_df[f"delta_{fan_id}"], errors="coerce").to_numpy(dtype=float)[order]
        ...
        params[fan_idx, 1] = float(np.interp(z_m, z_grid, d))
        params[fan_idx, 2] = float(np.interp(z_m, z_grid, w0))
        ...
        rows.append(
            {
                ...
                "A": float(params_mean[0]),
                "delta": float(params_mean[1]),
                "w0": float(params_mean[2]),
            }
        )
