...
import four_fan_gaussian_var_heat_map_main as model_plot
...
def main() -> None:
    ...
    for sheet_name in model_plot.SHEETS:
        ...
        for fan_idx in range(len(model_plot.FOUR_FAN_CENTERS_XY)):
            amp, delta, w0 = params[fan_idx]
            w_model += w0 + amp * np.exp(-(r_all[:, :, fan_idx] / delta) ** 2)

        out_png = (
            OUT_DIR
            / f"{sheet_name}_four_gaussian_var_heatmap_main_appendix.png"
        )
