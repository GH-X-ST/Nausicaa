...
- The parser searches for header cells equal to "variance" (case-insensitive).
  For each variance header, it also tries to recover the corresponding (x, y)
  (typically in the same block as columns "x", "y", "time", "w", "mean", "variance").
  It extracts one variance per representative point and sets:
      sigma_p(z) = sqrt(variance_p(z))
  For each radial bin j, a heteroscedastic scale is assigned by nearest radius:
      sigma_j(z) = sigma_{p*}(z),  p* = argmin_p |r_j - r_p(z)|
...
3) Fit parameters [A_r, r_p, sigma_r, w0] by bounded robust least squares
   ...
   Residuals are normalised by sigma_j(z) (estimated from *_TS by nearest radius),
   so f_scale can remain 1.0. The robust transition scale in physical units is
   ~sigma_j(z) (up to the alpha_j weighting).
...
class FitConfig:
    ...
    # Since residuals are normalised by sigma_j, keep f_scale ~ 1.
    robust_loss: str = "soft_l1"
    robust_f_scale: float = 1.0

    # If TS parsing fails, fallback sigma (m/s) used for all bins.
    sigma_z_fallback: float = 0.2

    # Floor on sigma to avoid overweighting if TS variance is tiny.
...
def parse_ts_points_and_sigmas(
    ...
    for r_idx in range(df.shape[0]):
        for c_idx in range(df.shape[1]):
            ...
            x_col = None
            if c_idx >= 5 and _cell_is_str(df, r_idx, c_idx - 5, "x") and _cell_is_str(df, r_idx, c_idx - 4, "y"):
                x_col = c_idx - 5
            else:
                # Fallback: search left for an "x" header followed by "y".
                left_start = max(0, c_idx - 12)
                for cc in range(c_idx - 1, left_start - 1, -1):
                    if _cell_is_str(df, r_idx, cc, "x") and cc + 1 < df.shape[1] and _cell_is_str(df, r_idx, cc + 1, "y"):
                        x_col = cc
                        break
    ...
    # Sort by radius; keep all points (typically 6).
...
def assign_sigma_bins_nearest(
    ...
    """
    Assign a heteroscedastic sigma_j to each radial bin by nearest-radius mapping.
    ...
    # Vectorised nearest-neighbour in 1D radius.
    diffs = np.abs(r_bins[:, None] - r_points[None, :])
    idx = np.argmin(diffs, axis=1)
    sigma_bins = sigma_points[idx].astype(float)

    # Safety floor (avoid overweighting due to tiny reported variance).
...
def fit_ring_at_height(
    ...
    Residual definition (dimensionless, heteroscedastic):
        res_j = alpha_j * (w_pred(r_j) - w_j) / sigma_j
    where alpha_j = sqrt(n_j / max(n)) and sigma_j is assigned per bin.

    With loss="soft_l1" and f_scale=1.0, the transition from quadratic to
    approximately linear occurs at |res_j| ~ 1, i.e. |w_pred - w_j| ~ sigma_j/alpha_j.
    """
    if sigma_bins.shape != r_bins.shape:
        raise ValueError("sigma_bins must have the same shape as r_bins.")
    if np.any(~np.isfinite(sigma_bins)) or np.any(sigma_bins <= 0.0):
        raise ValueError("sigma_bins must be positive and finite for all bins.")
    ...
    def residuals(p: np.ndarray) -> np.ndarray:
        ...
        return alpha * (w_pred - w_bins) / sigma_bins
...
def fit_all_heights(
    ...
    for sheet in mean_sheet_names:
        ...
        if ts_parsed is None:
            sigma_bins = np.full_like(r_bins, float(config.sigma_z_fallback), dtype=float)
            sigma_bins = np.maximum(sigma_bins, float(config.sigma_min))
        else:
