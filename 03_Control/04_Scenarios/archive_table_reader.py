...
from evidence_status import source_is_retired
    ...
    *,
    ...
    if fs_path.is_dir():
        ...
        if filesystem_path(manifest).is_file():
            ...
            return frame, _source_info(source=path, source_kind="run_root", manifest_path=manifest, frame=frame)
        partitions = sorted(item for item in fs_path.rglob("*") if item.is_file() and _is_partition_name(item.name))
        frame = _concat_partitions(partitions, max_rows=max_rows)
        return frame, _source_info(source=path, source_kind="partition_directory", manifest_path=None, frame=frame)
    if path.name == "table_manifest.json":
        frame = _read_manifest(path, max_rows=max_rows)
        return frame, _source_info(source=path, source_kind="table_manifest", manifest_path=path, frame=frame)
    if _is_partition_name(path.name):
        frame = read_table_partition(path)
        return frame.head(max_rows) if max_rows is not None else frame, _source_info(
            source=path,
...
def _read_manifest(path: Path, *, max_rows: int | None) -> pd.DataFrame:
    ...
    for partition in manifest.tables:
        frame = read_table_partition(root / "tables" / partition.relative_path, storage_format=partition.storage_format)
...
def _source_info(
    *,
    ...
    status, reason = _archive_evidence_status(source=source, frame=frame, run_manifest=run_manifest)
    return ArchiveTableSourceInfo(
        ...
        storage_format="" if table_manifest is None else str(table_manifest.storage_format),
        run_stage=str(run_manifest.get("run_stage", "unknown")),
        claim_status=str(run_manifest.get("claim_status", "unknown")),
        archive_evidence_status=status,
        evidence_eligibility_reason=reason,
    )
...
def _archive_evidence_status(
    *,
    ...
    if bool(run_manifest.get("dry_run_schedule", False)):
        return "blocked", "dry_run_schedule_no_dense_rows"
    if "primitive_variant_id" not in frame.columns:
        return "blocked", "missing_primitive_variant_id"
    return "complete", "w01_variant_rows_loaded"
...
def _run_manifest_path(manifest_path: Path | None) -> Path | None:
    if manifest_path is None:
        return None
    candidate = Path(manifest_path).parent / "run_manifest.json"
