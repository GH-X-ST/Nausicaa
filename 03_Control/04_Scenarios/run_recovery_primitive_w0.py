...
def run_smoke(run_id: int = 1, overwrite: bool = False) -> dict[str, Path]:
    """Run the W0 recovery primitive batch and write raw evidence outputs."""

    aircraft = adapt_glider(build_nausicaa_glider())
    result = run_recovery_batch(aircraft=aircraft)
    return write_recovery_outputs(
        result,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the W0 recovery primitive.")
