...
def _frame_from_axes(
    ...
    # Local element frames are rebuilt from body-axis directions to avoid hidden CAD state.
...
def rectangular_prism(
    ...
    # Prism inertia is computed in the local element frame before rotating to body axes.
...
def horizontal_surface_elements(
    ...
    # Horizontal surfaces are split left/right so dihedral and mirrored mass offsets are explicit.
...
def vertical_surface_element(
    ...
    # Body z is positive downward, so the vertical tail height points in negative body z.
...
def build_baseline_estimate() -> MassEstimate:
    ...
    # Component masses are deterministic hand estimates for the current Nausicaa build.
    elements = (
