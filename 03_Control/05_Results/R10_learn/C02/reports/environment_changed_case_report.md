...
- `claim_bearing_final_launch_count_positive`: `True` observed `160` required `>0_after_expected_low_energy_dry_air_sink_exclusion`
- `expected_low_energy_dry_air_sink_labeled`: `True` observed `40` required `diagnostic_not_claim_bearing_governor_failure`
- `hard_failure_rate_within_stage_profile`: `False` observed `0.1` required `0.01`
- `floor_or_ceiling_violation_rate_within_stage_profile`: `True` observed `0.0` required `0.0`
- `no_viable_primitive_rate_within_stage_profile`: `False` observed `0.1` required `0.02`
- `safe_success_rate_within_stage_profile`: `False` observed `0.8` required `0.99`
- `terminal_or_lift_capture_within_stage_profile`: `False` observed `0.8` required `0.9`
- `front_wall_mission_success_rate_diagnostic`: `True` observed `0.8` required `diagnostic_only_score_target_not_current_pass_gate`
- `wrong_wall_exit_rate_diagnostic`: `True` observed `0.1` required `diagnostic_only_penalised_by_launch_score`
- `selected_primitive_family_count_diagnostic`: `True` observed `8` required `diagnostic_only_not_a_governor_pass_gate`
- `selected_variant_count_diagnostic`: `True` observed `89` required `diagnostic_only_not_a_governor_pass_gate`
...
- `r10_learning_stage_uses_final_reject_rate_not_candidate_reject_rate`: `True` observed `0.1` required `bounded_final_no_viable_rate_plus_tuning_handoff_diagnostics`
