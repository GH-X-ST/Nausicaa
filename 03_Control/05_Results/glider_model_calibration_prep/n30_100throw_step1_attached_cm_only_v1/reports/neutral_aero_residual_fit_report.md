...
```powershell
python 03_Control/02_Inner_Loop/run_fit_neutral_aero_residual_calibration.py --session-root 04_Flight_Test\05_Results\cal\n30 --run-label n30_100throw_step1_attached_cm_only_v1 --heldout-count 10 --heldout-seed 606 --alignment-window-s 0.1 --derivative-window-s 0.04 --replay-dt-s 0.005 --ridge-lambda 0.001 --min-speed-m-s 1.5 --workers 8 --fit-workflow grouped_iterative --group-iterations 5 --group-improvement-tol 0.001 --aligned-u-min-m-s 4 --aligned-u-max-m-s 8 --aligned-v-abs-max-m-s 1.5 --aligned-w-abs-max-m-s 0.9 --filter-aligned-launch-state --apply-attached-cm-bias --no-fit-post-stall-longitudinal --no-fit-transition-blender --no-fit-post-stall-surfaces --no-fit-post-stall-damping --no-fit-attached-lateral-coupling --no-fit-transition-lateral-coupling --no-fit-lateral-surfaces --no-fit-secondary-lateral-diagnostic
```
...
- grouped history rows: `15`
- grouped history CSV: `metrics/neutral_aero_residual_group_iteration_history.csv`
- selected grouped replay scales:
  - pass `0`, group `initial_grouped_candidate`, scale ``, objective `39.2576`
  - pass `1`, group `attached_longitudinal`, scale `1.25`, objective `27.1827`
  - pass `2`, group `attached_longitudinal`, scale `1.25`, objective `27.1827`
...
- lateral diagnostic policy: secondary lateral diagnostic is reportable only if held-out dy, roll, and yaw improve versus the primary longitudinal candidate while dx, altitude loss, sink, and pitch do not degrade
  - dy_mae_m (lateral_improvement_required): primary `1.7764`, diagnostic `nan`, delta `nan`, pass `False`
  - final_phi_mae_deg (lateral_improvement_required): primary `27.2255`, diagnostic `nan`, delta `nan`, pass `False`
  - final_psi_mae_deg (lateral_improvement_required): primary `34.3803`, diagnostic `nan`, delta `nan`, pass `False`
  - dx_mae_m (longitudinal_preservation_required): primary `0.5383`, diagnostic `nan`, delta `nan`, pass `False`
  - altitude_loss_mae_m (longitudinal_preservation_required): primary `0.2239`, diagnostic `nan`, delta `nan`, pass `False`
  - sink_mae_m_s (longitudinal_preservation_required): primary `0.2049`, diagnostic `nan`, delta `nan`, pass `False`
  - final_theta_mae_deg (longitudinal_preservation_required): primary `12.9015`, diagnostic `nan`, delta `nan`, pass `False`

## Replay Validation

- baseline train pitch MAE: `22.511` deg
- candidate train pitch MAE: `16.124` deg
- baseline held-out pitch MAE: `21.007` deg
- candidate held-out pitch MAE: `12.902` deg
- baseline held-out altitude-loss MAE: `1.2741` m
- candidate held-out altitude-loss MAE: `0.2239` m
- baseline held-out dx MAE: `0.9664` m
- candidate held-out dx MAE: `0.5383` m
...
- acceptance policy: primary longitudinal candidate must improve or preserve held-out dx, altitude loss, sink, and pitch MAE versus baseline_active; lateral errors are reported but not claim-bearing
  - dx_mae_m: baseline `0.9664`, candidate `0.5383`, delta `-0.4281`, pass `True`
  - altitude_loss_mae_m: baseline `1.2741`, candidate `0.2239`, delta `-1.0502`, pass `True`
  - sink_mae_m_s: baseline `1.1373`, candidate `0.2049`, delta `-0.9325`, pass `True`
  - final_theta_mae_deg: baseline `21.0070`, candidate `12.9015`, delta `-8.1055`, pass `True`
...
- train/attached: samples `2070`, throws `57`
  ...
  - candidate dx/dy/alt/sink/roll/pitch/yaw MAE: `0.131` m, `0.473` m, `0.092` m, `0.195` m/s, `9.60` deg, `7.67` deg, `10.78` deg
- train/transition: samples `6603`, throws `57`
  ...
  - candidate dx/dy/alt/sink/roll/pitch/yaw MAE: `0.133` m, `0.591` m, `0.151` m, `0.271` m/s, `13.23` deg, `9.95` deg, `14.60` deg
- train/post_stall: samples `916`, throws `19`
  ...
  - candidate dx/dy/alt/sink/roll/pitch/yaw MAE: `0.141` m, `0.965` m, `0.109` m, `0.172` m/s, `16.03` deg, `9.74` deg, `13.77` deg
- heldout/attached: samples `357`, throws `10`
  ...
  - candidate dx/dy/alt/sink/roll/pitch/yaw MAE: `0.106` m, `0.556` m, `0.051` m, `0.171` m/s, `10.24` deg, `6.23` deg, `10.69` deg
- heldout/transition: samples `1300`, throws `10`
  ...
  - candidate dx/dy/alt/sink/roll/pitch/yaw MAE: `0.139` m, `0.717` m, `0.155` m, `0.263` m/s, `15.51` deg, `10.59` deg, `16.59` deg
- heldout/post_stall: samples `121`, throws `3`
  ...
  - candidate dx/dy/alt/sink/roll/pitch/yaw MAE: `0.262` m, `1.134` m, `0.311` m, `0.381` m/s, `22.53` deg, `7.19` deg, `17.90` deg
...
- train/transition_before_post_stall: count `5758`, Cm mean `0.03271`, Cm MAE `0.04376`, Cm fit residual MAE `0.04488`, CY mean `-0.08358`, Cl mean `-0.00965`, Cn mean `0.00326`
- train/transition_after_post_stall: count `845`, Cm mean `0.04012`, Cm MAE `0.05820`, Cm fit residual MAE `0.05840`, CY mean `0.06761`, Cl mean `-0.00447`, Cn mean `0.00441`
- train/post_stall: count `916`, Cm mean `-0.00669`, Cm MAE `0.05599`, Cm fit residual MAE `0.05570`, CY mean `-0.07054`, Cl mean `-0.02602`, Cn mean `0.00836`
...
- heldout/transition_before_post_stall: count `1200`, Cm mean `0.03134`, Cm MAE `0.04177`, Cm fit residual MAE `0.04316`, CY mean `-0.09275`, Cl mean `-0.01313`, Cn mean `0.00556`
- heldout/transition_after_post_stall: count `100`, Cm mean `0.02934`, Cm MAE `0.04702`, Cm fit residual MAE `0.04795`, CY mean `0.12938`, Cl mean `-0.01400`, Cn mean `0.00688`
- heldout/post_stall: count `121`, Cm mean `-0.02355`, Cm MAE `0.04547`, Cm fit residual MAE `0.04255`, CY mean `-0.03690`, Cl mean `-0.04273`, Cn mean `0.01100`
