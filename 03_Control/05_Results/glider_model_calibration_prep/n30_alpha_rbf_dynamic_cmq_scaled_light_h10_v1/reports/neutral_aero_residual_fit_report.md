...
```powershell
python 03_Control/02_Inner_Loop/run_fit_neutral_aero_residual_calibration.py --session-root 04_Flight_Test\05_Results\cal\n30 --run-label n30_alpha_rbf_dynamic_cmq_scaled_light_h10_v1 --heldout-count 10 --heldout-seed 606 --alignment-window-s 0.1 --derivative-window-s 0.04 --replay-dt-s 0.005 --ridge-lambda 0.001 --min-speed-m-s 1.5 --workers 8 --no-apply-attached-cm-bias --fit-post-stall-damping
```
...
- candidate train pitch MAE: `9.718` deg
- baseline held-out pitch MAE: `18.886` deg
- candidate held-out pitch MAE: `15.210` deg
- baseline held-out altitude-loss MAE: `0.3265` m
- candidate held-out altitude-loss MAE: `0.3267` m
- baseline held-out dx MAE: `0.7509` m
- candidate held-out dx MAE: `0.6987` m
...
- train/attached: samples `116`, throws `14`
  ...
  - candidate dx/dy/alt/sink/roll/pitch/yaw MAE: `0.097` m, `0.009` m, `0.011` m, `0.293` m/s, `0.66` deg, `2.27` deg, `0.20` deg
- train/transition: samples `532`, throws `20`
  ...
  - candidate dx/dy/alt/sink/roll/pitch/yaw MAE: `0.078` m, `0.105` m, `0.063` m, `0.326` m/s, `3.75` deg, `4.35` deg, `1.03` deg
- train/post_stall: samples `2851`, throws `20`
  ...
  - candidate dx/dy/alt/sink/roll/pitch/yaw MAE: `0.295` m, `0.602` m, `0.214` m, `0.335` m/s, `24.29` deg, `10.29` deg, `9.26` deg
...
- heldout/transition: samples `265`, throws `10`
  ...
  - candidate dx/dy/alt/sink/roll/pitch/yaw MAE: `0.203` m, `0.245` m, `0.172` m, `0.435` m/s, `7.25` deg, `9.60` deg, `3.84` deg
- heldout/post_stall: samples `1420`, throws `10`
  ...
  - candidate dx/dy/alt/sink/roll/pitch/yaw MAE: `0.363` m, `0.805` m, `0.230` m, `0.334` m/s, `26.76` deg, `10.82` deg, `8.63` deg
...
- post-stall CL surface: 20 deg baseline `0` -> candidate `0.179121`, 45 deg baseline `0` -> candidate `-0.155357`, 70 deg baseline `0` -> candidate `-0.0772501`
- post-stall CD surface: 20 deg baseline `0` -> candidate `0.056022`, 45 deg baseline `0` -> candidate `0.00805644`, 70 deg baseline `0` -> candidate `-0.409247`
- post-stall Cm surface: 20 deg baseline `0` -> candidate `-0.0123101`, 45 deg baseline `0` -> candidate `-0.0502331`, 70 deg baseline `0` -> candidate `0.0162973`
- post-stall Cmq surface: 20 deg baseline `0` -> candidate `2`, 45 deg baseline `0` -> candidate `-0.918346`, 70 deg baseline `0` -> candidate `2`
