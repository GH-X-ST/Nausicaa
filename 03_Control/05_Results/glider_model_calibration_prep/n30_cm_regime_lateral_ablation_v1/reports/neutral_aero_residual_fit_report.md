...
- policy: longitudinal candidate is frozen; each diagnostic candidate fits one lateral term and one regime family at a time.
...
- held-out ablations tested: `9`
- accepted held-out lateral ablations:
  - `attached_CY_beta` coeff `-1.18595` (negative); dy `1.612` -> `0.805` m, roll `28.16` -> `23.56` deg, yaw `27.10` -> `10.64` deg
- best held-out dy reductions, even if rejected:
  - `attached_CY_beta` coeff `-1.18595` delta dy `-0.807` m, delta roll `-4.60` deg, delta yaw `-16.46` deg, reason `heldout_lateral_improved_with_longitudinal_tolerance`
  - `attached_Cl_p` coeff `0.25406` delta dy `-0.364` m, delta roll `1.50` deg, delta yaw `-12.90` deg, reason `rejected_lateral_metrics_not_all_improved`
  - `post_stall_Cl_p` coeff `0.556542` delta dy `-0.121` m, delta roll `6.07` deg, delta yaw `0.02` deg, reason `rejected_lateral_metrics_not_all_improved`
  - `attached_Cn_r` coeff `0.290767` delta dy `-0.004` m, delta roll `-5.95` deg, delta yaw `55.92` deg, reason `rejected_lateral_metrics_not_all_improved`
  - `transition_CY_beta` coeff `0.020079` delta dy `0.005` m, delta roll `-0.00` deg, delta yaw `0.15` deg, reason `rejected_lateral_metrics_not_all_improved`
