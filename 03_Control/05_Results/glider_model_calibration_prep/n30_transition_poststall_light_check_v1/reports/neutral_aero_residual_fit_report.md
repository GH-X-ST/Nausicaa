...
```powershell
python 03_Control/02_Inner_Loop/run_fit_neutral_aero_residual_calibration.py --session-root 04_Flight_Test\05_Results\cal\n30 --run-label n30_transition_poststall_light_check_v1 --heldout-count 6 --heldout-seed 606 --alignment-window-s 0.1 --derivative-window-s 0.04 --replay-dt-s 0.005 --ridge-lambda 0.001 --min-speed-m-s 1.5 --workers 8 --no-apply-attached-cm-bias --no-fit-post-stall-damping
```
...
- candidate train pitch MAE: `14.784` deg
- baseline held-out pitch MAE: `15.825` deg
- candidate held-out pitch MAE: `17.085` deg
- baseline held-out altitude-loss MAE: `0.4567` m
- candidate held-out altitude-loss MAE: `0.6544` m
- baseline held-out dx MAE: `0.9731` m
- candidate held-out dx MAE: `1.3871` m
