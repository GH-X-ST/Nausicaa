...
- Residual memory policy: `outer_loop_baseline_shielded_recency_safe_exploration_memory_v1_2` applies capped, recency-weighted candidate-path residual memory and shielded uncertainty exploration after viability filtering.
...
- `hard_failure_rate_within_stage_profile`: `True` observed `0.075` required `0.2`
...
- `safe_success_rate_within_stage_profile`: `True` observed `0.925` required `0.2`
...
- `selected_variant_count_diagnostic`: `True` observed `40` required `diagnostic_only_not_a_governor_pass_gate`
