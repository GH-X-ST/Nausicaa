...
# 3) Recovery case construction
# 4) Closed-loop recovery rollout and batch execution
# 5) Output writing
# =============================================================================
...
RECOVERY_CAMPAIGN = "05_recovery_primitive"
GLIDE_PROXY_SOURCE = "build_glide_primitive_spec + evaluate_entry_set"
RECOVERY_NOTES = "recovery_w0_local_feedback_handoff_to_glide_proxy"
VALIDATION_COMMANDS = (
    "python -m py_compile "
    "03_Control/03_Primitives/recovery_primitive.py "
    "03_Control/04_Scenarios/run_recovery_primitive_w0.py",
    "python 03_Control/04_Scenarios/run_recovery_primitive_w0.py "
    "--run-id 1 --overwrite",
    "python -m pytest -q "
    "03_Control/tests/test_recovery_primitive.py "
    "03_Control/tests/test_recovery_primitive_w0.py",
    "python -m pytest -q 03_Control/tests",
)


@dataclass(frozen=True)
class RecoveryFeedbackGains:
    k_phi: float = 0.75
    k_p: float = 0.18
    k_theta: float = 0.65
    k_q: float = 0.18
...
@dataclass(frozen=True)
class RecoveryPrimitiveConfig:
    dt_s: float = 0.02
    t_final_s: float = 0.64
    speed_m_s: float = 6.5
    ...
    seed: int = 1
    scenario_name: str = "recovery_w0_batch"
...
class RecoveryCaseResult:
    ...
    recovery_checks: tuple[RecoveryCheckResult, ...]
...
class RecoveryBatchResult:
    primitive_spec: PrimitiveSpec
    case_results: tuple[RecoveryCaseResult, ...]
    overall_status: str
...
def _validate_config(config: RecoveryPrimitiveConfig) -> None:
    if not np.isfinite(float(config.dt_s)) or float(config.dt_s) <= 0.0:
        raise ValueError("recovery dt_s must be finite and positive.")
    if not np.isfinite(float(config.t_final_s)) or float(config.t_final_s) <= 0.0:
        raise ValueError("recovery t_final_s must be finite and positive.")
    _time_step_count(config.dt_s, config.t_final_s)
    if not np.isfinite(float(config.speed_m_s)) or float(config.speed_m_s) <= 0.0:
        raise ValueError("recovery speed_m_s must be finite and positive.")
    if not np.isfinite(float(config.altitude_m)):
        raise ValueError("recovery altitude_m must be finite.")
    if config.wind_mode != "none":
        raise ValueError("W0 recovery supports only wind_mode='none'.")
    if config.latency_case != "none":
        raise ValueError("W0 recovery supports only latency_case='none'.")
...
def _solve_trim(config: RecoveryPrimitiveConfig, aircraft: object) -> TrimResult:
...
def _state_speed_alpha_beta(state: np.ndarray) -> tuple[float, float, float]:
    vector = as_state_vector(state)
    speed = float(np.linalg.norm(vector[6:9]))
    alpha_rad = float(np.arctan2(vector[STATE_INDEX["w"]], vector[STATE_INDEX["u"]]))
    beta_rad = 0.0
    if speed > 1e-9:
...
def recovery_feedback_command_norm(
    x: np.ndarray,
    x_ref: np.ndarray,
    u_trim_norm: np.ndarray,
    gains: RecoveryFeedbackGains,
) -> np.ndarray:
    """Return requested normalised aggregate command for W0 recovery feedback."""

    state = as_state_vector(x)
    reference = as_state_vector(x_ref)
    trim_norm = as_normalised_command_vector(u_trim_norm)
    speed, alpha_rad, beta_rad = _state_speed_alpha_beta(state)
    speed_ref, alpha_ref, _ = _state_speed_alpha_beta(reference)
    ...
    alpha_error = float(alpha_rad - alpha_ref)
    ...
    if not np.all(np.isfinite(requested)):
        raise ValueError("recovery feedback produced a nonfinite command.")
    return requested
...
# 3) Recovery Case Construction
# =============================================================================
def build_recovery_primitive_spec(
    config: RecoveryPrimitiveConfig | None = None,
) -> PrimitiveSpec:
    """Return the W0 recovery primitive metadata contract."""

    cfg = RecoveryPrimitiveConfig() if config is None else config
    _validate_config(cfg)
    entry_set = PrimitiveEntrySet(
        name="recovery_w0_entry",
        description="W0 recovery entry set in SI units and radians",
        lower={
            "x_w": 1.2,
            "y_w": 0.3,
            "z_w": 0.8,
    ...
    exit_checks = (
        PrimitiveExitCheck(
            name="finite_state",
            description="full recovery state history remains finite",
            required=True,
        ),
        PrimitiveExitCheck(
        ...
        PrimitiveExitCheck(
            ...
            description="closed-loop recovery rollout integrity metric reports success",
            required=True,
        ),
    )
    spec = PrimitiveSpec(
        name="recovery_w0_nominal",
...
def _base_recovery_state(trim_result: TrimResult, config: RecoveryPrimitiveConfig) -> np.ndarray:
    if not trim_result.converged:
        raise ValueError("cannot build recovery cases from nonconverged trim.")
    state = as_state_vector(trim_result.x_trim).copy()
    state[0:3] = [1.3, 2.2, float(config.altitude_m)]
    return state
...
def build_recovery_cases(
...
) -> tuple[RecoveryCaseSpec, ...]:
    """Return deterministic W0 recovery case inventory."""
    ...
    base = _base_recovery_state(trim, cfg)

    required = base.copy()
    required[STATE_INDEX["phi"]] += 0.22
    required[STATE_INDEX["theta"]] += 0.12
    required[STATE_INDEX["p"]] = -0.35
    required[STATE_INDEX["q"]] = 0.28
    required[STATE_INDEX["r"]] = 0.20

    low_speed = base.copy()
    low_speed = _set_velocity_from_speed_angles(low_speed, 5.9, 0.12, 0.0)
    low_speed[STATE_INDEX["theta"]] += 0.14
    low_speed[STATE_INDEX["q"]] = 0.18

    sideslip = base.copy()
    sideslip = _set_velocity_from_speed_angles(sideslip, 6.3, 0.07, 0.14)
    sideslip[STATE_INDEX["phi"]] += 0.10
    sideslip[STATE_INDEX["psi"]] += 0.08
    sideslip[STATE_INDEX["r"]] = 0.50

    high_rate = base.copy()
    high_rate[STATE_INDEX["phi"]] += 0.45
    high_rate[STATE_INDEX["theta"]] += 0.28
    high_rate[STATE_INDEX["p"]] = 1.15
    high_rate[STATE_INDEX["q"]] = -1.10
    high_rate[STATE_INDEX["r"]] = 0.90

    boundary = required.copy()
    return (
        RecoveryCaseSpec(
            name="recovery_w0_moderate_attitude_rate",
            role="required",
            description="required W0 recovery case with moderate attitude and rate error",
            x0=required,
            t_final_s=float(cfg.t_final_s),
        ),
        RecoveryCaseSpec(
            name="recovery_w0_low_speed_pitch_up",
            role="optional",
            description="optional lower-speed pitch-up recovery coverage",
            x0=low_speed,
            t_final_s=float(cfg.t_final_s),
        ),
        RecoveryCaseSpec(
            name="recovery_w0_sideslip_yaw_rate",
            role="optional",
            description="optional sideslip and yaw-rate recovery coverage",
            x0=sideslip,
            t_final_s=float(cfg.t_final_s),
        ),
        RecoveryCaseSpec(
            name="recovery_w0_high_rate_boundary",
            role="diagnostic",
            description="diagnostic high-rate boundary recovery case",
            x0=high_rate,
            t_final_s=float(cfg.t_final_s),
        ),
        RecoveryCaseSpec(
            name="recovery_w0_0p80_boundary",
            role="diagnostic",
...
def _recovery_checks(
    ...
    terminal_alpha_deg = float(abs(np.rad2deg(alpha_rad[-1])))
    terminal_beta_deg = float(abs(np.rad2deg(beta_rad[-1])))
    terminal_bank_deg = float(abs(np.rad2deg(x_log[-1, STATE_INDEX["phi"]])))
    terminal_pitch_deg = float(abs(np.rad2deg(x_log[-1, STATE_INDEX["theta"]])))
    ...
    return (
        _check(
            ...
            "m/s",
            "terminal_speed_above_handoff_minimum",
        ),
        _check(
            "terminal_speed_max",
            terminal_speed_m_s,
            ...
            "m/s",
            "terminal_speed_below_handoff_maximum",
        ),
        _check(
            "terminal_alpha_boundary",
            terminal_alpha_deg,
            15.0,
            terminal_alpha_deg <= 15.0,
            "deg",
            "terminal_alpha_within_recovery_handoff_bound",
        ),
        _check(
            "terminal_beta_boundary",
            terminal_beta_deg,
            10.0,
            terminal_beta_deg <= 10.0,
            "deg",
            "terminal_beta_within_recovery_handoff_bound",
        ),
        _check(
            "terminal_bank_boundary",
            terminal_bank_deg,
            12.0,
            terminal_bank_deg <= 12.0,
            "deg",
            "terminal_bank_within_recovery_handoff_bound",
        ),
        _check(
            "terminal_pitch_boundary",
            terminal_pitch_deg,
            20.0,
            terminal_pitch_deg <= 20.0,
            "deg",
            "terminal_pitch_within_recovery_handoff_bound",
        ),
        _check(
            "terminal_rate_boundary",
            terminal_rate_rad_s,
            0.6,
            terminal_rate_rad_s <= 0.6,
            "rad/s",
            "terminal_body_rate_norm_within_recovery_handoff_bound",
        ),
        _check(
            "max_alpha_boundary",
            ...
            "deg",
            "trajectory_alpha_within_w0_recovery_bound",
        ),
        _check(
            "max_beta_boundary",
            ...
            "deg",
            "trajectory_beta_within_w0_recovery_bound",
        ),
        _check(
            "max_rate_boundary",
            ...
            "rad/s",
            "trajectory_rate_within_w0_recovery_bound",
        ),
        _check(
            "saturation_fraction",
...
def _first_failed_recovery_label(checks: tuple[RecoveryCheckResult, ...]) -> str:
    mapping = {
        "terminal_speed_min": "speed_low",
        "terminal_speed_max": "speed_high",
        "terminal_alpha_boundary": "alpha_boundary",
        "terminal_beta_boundary": "beta_boundary",
...
def _boundary_x_limited_only(case: RecoveryCaseSpec, x_terminal: np.ndarray) -> bool:
    if case.name != "recovery_w0_0p80_boundary":
        return False
    failures = _glide_entry_failures(x_terminal)
    return bool(failures) and {failure.variable for failure in failures} == {"x_w"}


def rollout_recovery_case(
    case: RecoveryCaseSpec,
    config: RecoveryPrimitiveConfig | None = None,
    gains: RecoveryFeedbackGains | None = None,
    aircraft: object | None = None,
    wind_model: object = None,
) -> RecoveryCaseResult:
    """Run one W0 recovery case with local feedback and exact glide handoff check."""

    cfg = RecoveryPrimitiveConfig() if config is None else config
    _validate_config(cfg)
    if wind_model is not None:
        raise ValueError("W0 recovery requires wind_model=None.")
    feedback_gains = RecoveryFeedbackGains() if gains is None else gains
    aircraft_model = _default_aircraft() if aircraft is None else aircraft
    spec = build_recovery_primitive_spec(cfg)
    ...
    recovery_checks = _recovery_checks(case, x_log, preliminary_metrics, glide_spec)
    recovery_pass = bool(
        all(check.pass_check for check in recovery_checks if check.required)
    )
    primitive_success = bool(entry_pass and exit_pass and recovery_pass)
    if primitive_success:
        final_failure_label = "success"
        final_notes = RECOVERY_NOTES
    elif rollout_failure_label != "not_run":
        final_failure_label = rollout_failure_label
        final_notes = rollout_failure_label
    elif _boundary_x_limited_only(case, x_log[-1]):
        final_failure_label = "terminal_recovery_limited"
        final_notes = "glide_entry_x_bound_limited"
    elif not exit_pass:
        final_failure_label = "terminal_recovery_limited"
        final_notes = "recovery_exit_check_failure"
    else:
        final_failure_label = _first_failed_recovery_label(recovery_checks)
...
def run_recovery_batch(
    ...
    """Run the W0 recovery case inventory and summarise acceptance status."""

    cfg = RecoveryPrimitiveConfig() if config is None else config
    _validate_config(cfg)
    aircraft_model = _default_aircraft() if aircraft is None else aircraft
    trim = _solve_trim(cfg, aircraft_model)
    cases = build_recovery_cases(cfg, trim)
    results = tuple(
    ...
    if not required_success:
        ...
        notes = "required_recovery_case_failed"
    elif optional_failures:
        overall_status = "pass_with_optional_case_failures"
        failure_label = "success"
        notes = "required_recovery_case_passed_with_optional_failures"
    else:
        overall_status = "pass"
        failure_label = "success"
        notes = "required_recovery_case_passed"
    return RecoveryBatchResult(
        primitive_spec=build_recovery_primitive_spec(cfg),
...
def _recovery_checks_dataframe(checks: tuple[RecoveryCheckResult, ...]) -> pd.DataFrame:
    return pd.DataFrame([check.__dict__ for check in checks])


def _case_summary_dataframe(batch: RecoveryBatchResult) -> pd.DataFrame:
    rows: list[dict[str, object]] = []
    for result in batch.case_results:
        terminal_x = float(result.x[-1, STATE_INDEX["x_w"]])
        x_margin = terminal_glide_entry_x_margin_m(result.x[-1])
        terminal_proxy = terminal_glide_entry_proxy(result.x[-1])
        rows.append(
            {
                ...
                "terminal_x_w_m": terminal_x,
                "terminal_glide_entry_x_margin_m": x_margin,
                "terminal_glide_entry_proxy": terminal_proxy,
                "terminal_glide_entry_proxy_source": GLIDE_PROXY_SOURCE,
            }
        )
...
def write_recovery_outputs(
    ...
    """Write W0 recovery primitive raw CSV/JSON/Markdown evidence."""

    paths = make_result_tree(Path(result_root), campaign, run_id, overwrite=overwrite)
    suffix = f"s{run_id:03d}"
    output_paths: dict[str, Path] = {
        "case_summary_csv": paths["metrics"] / f"recovery_case_summary_{suffix}.csv",
        "manifest_json": paths["manifests"] / f"recovery_primitive_manifest_{suffix}.json",
        "report_md": paths["reports"] / f"recovery_primitive_report_{suffix}.md",
    }
    ...
    for case_result in result.case_results:
        ...
        output_paths[f"{case_name}_recovery_checks_csv"] = (
            paths["metrics"] / f"{case_name}_recovery_checks_{suffix}.csv"
        )
        ...
        _recovery_checks_dataframe(case_result.recovery_checks).to_csv(
            output_paths[f"{case_name}_recovery_checks_csv"],
            index=False,
        )
