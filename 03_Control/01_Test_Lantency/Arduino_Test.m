...
for commandIndex = 1:rowCount
    ...
    if ismember("computer_to_arduino_rx_latency_s", string(echoImportTable.Properties.VariableNames))
        echoAssignments.computer_to_arduino_rx_latency_s(commandIndex) = ...
            echoImportTable.computer_to_arduino_rx_latency_s(matchedRow);
    end

    if ismember("computer_to_arduino_apply_latency_s", string(echoImportTable.Properties.VariableNames))
        echoAssignments.computer_to_arduino_apply_latency_s(commandIndex) = ...
            echoImportTable.computer_to_arduino_apply_latency_s(matchedRow);
    end

    if ismember("arduino_receive_to_apply_latency_s", string(echoImportTable.Properties.VariableNames))
        echoAssignments.arduino_receive_to_apply_latency_s(commandIndex) = ...
            echoImportTable.arduino_receive_to_apply_latency_s(matchedRow);
    end

    echoAssignments.applied_position(commandIndex) = ...
        echoImportTable.applied_position(matchedRow);

    echoAssignments.applied_equivalent_deg(commandIndex) = ...
        echoImportTable.applied_equivalent_deg(matchedRow);
end
end
...
for surfaceIndex = 1:surfaceCount
    ...
    rxValuesSeconds = storage.computerToArduinoRxLatencySeconds(sampleIndices, surfaceIndex);
    rxValuesSeconds = rxValuesSeconds(isfinite(rxValuesSeconds));

    applyValuesSeconds = storage.computerToArduinoApplyLatencySeconds(sampleIndices, surfaceIndex);
    applyValuesSeconds = applyValuesSeconds(isfinite(applyValuesSeconds));

    receiveToApplyValuesSeconds = storage.arduinoReceiveToApplyLatencySeconds(sampleIndices, surfaceIndex);
    receiveToApplyValuesSeconds = receiveToApplyValuesSeconds(isfinite(receiveToApplyValuesSeconds));

    matchedRxCommandCount(surfaceIndex) = numel(rxValuesSeconds);
    ...
    matchedApplyCommandCount(surfaceIndex) = numel(applyValuesSeconds);
    ...
    [rxMeanSeconds(surfaceIndex), rxStdSeconds(surfaceIndex), ...
        rxMedianSeconds(surfaceIndex), rxP95Seconds(surfaceIndex), rxMaxSeconds(surfaceIndex)] = ...
        computeLatencyStats(rxValuesSeconds);

    [applyMeanSeconds(surfaceIndex), applyStdSeconds(surfaceIndex), ...
        applyMedianSeconds(surfaceIndex), applyP95Seconds(surfaceIndex), applyMaxSeconds(surfaceIndex)] = ...
        computeLatencyStats(applyValuesSeconds);

    [receiveToApplyMeanSeconds(surfaceIndex), receiveToApplyStdSeconds(surfaceIndex), ...
        receiveToApplyMedianSeconds(surfaceIndex), receiveToApplyP95Seconds(surfaceIndex), receiveToApplyMaxSeconds(surfaceIndex)] = ...
        computeLatencyStats(receiveToApplyValuesSeconds);
end
...
latencySummary = table( ...
    ...
    rxMeanSeconds, rxStdSeconds, rxMedianSeconds, rxP95Seconds, rxMaxSeconds, ...
    applyMeanSeconds, applyStdSeconds, applyMedianSeconds, applyP95Seconds, applyMaxSeconds, ...
    receiveToApplyMeanSeconds, receiveToApplyStdSeconds, receiveToApplyMedianSeconds, receiveToApplyP95Seconds, receiveToApplyMaxSeconds, ...
    'VariableNames', { ...
        'SurfaceName', ...
        'IsActive', ...
        'DispatchedCommandCount', ...
...
function [meanValue, stdValue, medianValue, p95Value, maxValue] = computeLatencyStats(values)
meanValue = nan;
stdValue = nan;
medianValue = nan;
p95Value = nan;
...
medianValue = computeSamplePercentile(values, 50);
p95Value = computeSamplePercentile(values, 95);
maxValue = max(values);
end
