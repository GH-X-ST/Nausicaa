...
config.debug = struct( ...
    "strictDownstreamMatching", getLogicalField(debugConfig, "strictDownstreamMatching", true));
...
commitRows = matchedEvents(isfinite(matchedEvents.board_commit_s) & isfinite(matchedEvents.expected_ppm_us), :);
commitRows = sortrows(commitRows, {'sample_sequence', 'surface_index'});
commitTransitionTables = buildMatchedEventCommitTransitionTablesBySurface( ...
    commitRows, ...
    config.surfaceNames, ...
...
transitionAssociationWindowSeconds = max( ...
    1.0, ...
    config.matching.maxResponseWindowSeconds + max(0.0, config.matching.transitionLeadSeconds) + 0.05);
...
for rowIndex = 1:height(matchedEvents)
    ...
    trainerSearchStartSeconds = applyEstimatedTransitionLag( ...
        searchAnchorSeconds, ...
        trainerLagBySurface(surfaceIdx));

    if matchedEvents.is_observable_downstream(rowIndex) && ...
            isfinite(previousState) && isfinite(expectedState) && previousState ~= expectedState
        ...
        receiverStartSeconds = applyEstimatedTransitionLag( ...
            searchAnchorSeconds, ...
            receiverLagBySurface(surfaceIdx));
        ...
        receiverPreviousState = classifyPulseState( ...
            previousExpectedUs(surfaceIdx), ...
...
for surfaceIndex = 1:numel(surfaceNames)
    ...
    elseif numel(pulseValues) == 2
        stateCentersBySurface{surfaceIndex} = [pulseValues(1), mean(pulseValues), pulseValues(2)];
...
for commitIndex = 2:height(commitUnique)
    ...
    for surfaceIndex = 1:numel(config.surfaceNames)
        ...
        trainerSearchStartSeconds = applyEstimatedTransitionLag(anchorTime, trainerLagBySurface(surfaceIndex));
        ...
        % receiver waveform.
        receiverSearchStartSeconds = applyEstimatedTransitionLag(anchorTime, receiverLagBySurface(surfaceIndex));
...
if isempty(transitionTable) || ~isfinite(previousState) || ~isfinite(expectedState) || previousState == expectedState
    return;
end
...
truthSubsetEvents = buildTruthSubsetLatencyEvents( ...
    ...
    config);
if config.debug.strictDownstreamMatching
    validateStrictDownstreamMatching(directLatencyEvents, truthSubsetEvents);
end
...
for surfaceIndex = 1:surfaceCount
    ...
    boardCommitToReference = surfaceRows.reference_strobe_s - surfaceRows.board_commit_s;
    referenceToTrainer = surfaceRows.trainer_transition_s - surfaceRows.reference_strobe_s;
    trainerToReceiver = surfaceRows.receiver_transition_s - surfaceRows.trainer_transition_s;
