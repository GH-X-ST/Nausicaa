...
%   runData = Transmitter_Test(config) drives the Nano 33 IoT over serial,
...
try
    ...
    runData.surfaceSummary = buildTransmitterSurfaceSummary(storage, config);
...
config.arduinoTransport = struct( ...
    "mode", "nano_transmitter_serial", ...
    "serialPort", getTextScalarField(transportConfig, "serialPort", "COM5"), ...
    "baudRate", getPositiveIntegerField(transportConfig, "baudRate", 460800), ...
    "commandEncoding", canonicalizeNanoLoggerCommandEncoding( ...
...
try
    ...
    commandInterface = struct( ...
        "transportMode", "nano_transmitter_serial", ...
    ...
    connectionInfo.loggerFirmwareVersion = firmwareVersion;
    connectionInfo.connectionMessage = "Connected over serial and received HELLO_EVENT.";
catch connectionException
...
function latencySummary = buildTransmitterLatencySummary(storage, config)
surfaceCount = numel(config.surfaceNames);
...
for surfaceIndex = 1:surfaceCount
    ...
    ppmToReceiverStats(surfaceIndex) = computeLatencyStats(storage.ppmToReceiverLatencySeconds(:, surfaceIndex));
    computerToReceiverStats(surfaceIndex) = computeLatencyStats(storage.computerToReceiverLatencySeconds(:, surfaceIndex));
    scheduledToReceiverStats(surfaceIndex) = computeLatencyStats(storage.scheduledToReceiverLatencySeconds(:, surfaceIndex));
end
