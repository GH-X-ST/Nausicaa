...
# Surface order follows the direct-servo output capture channels generated by
# Arduino_Test_E2E_Post; the labels are human-facing only.
SURFACE_STYLES = [
...
# Latency columns are already post-processed in seconds and represent the
# controller-to-servo chain used for the E2E comparison.
...
def _resolve_logger_folder(logger_folder: str | None, seed: int | None) -> Path:
    if logger_folder:
        ...
        return path
    # Seed-based folders are deterministic run artifacts; requiring one avoids
    # accidentally plotting an unrelated latest run in reproducibility figures.
